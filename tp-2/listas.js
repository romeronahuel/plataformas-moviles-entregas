...
function encontrarPalabraMasCorta(listaDePalabras) {
    return listaDePalabras.reduce((shortest, word) => {
        return word.length < shortest.length ? word : shortest;
    });
}

console.log("encontrarPalabraMasCorta(['abc', 'a', 'ab', 'c']): ", encontrarPalabraMasCorta(['abc', 'a', 'ab', 'c']));
...
function enumerarLista(listaDePalabras) {
    ...
    const lastIndex = listaDePalabras.length - 1;
    const firstPart = listaDePalabras.slice(0, lastIndex).join(', ');
    const lastWord = listaDePalabras[lastIndex];

    return `${firstPart} y ${lastWord}.`;
}
